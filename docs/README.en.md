...
<div align="center">
    <img src="./img/logo.png" alt="niko-link" style="border-radius: 16px; width: 25%; display: block; margin: 0 auto;" />
</div>

<h1>NekoLink</h1>

<p>
  A native macOS proxy client, powered by
  <a href="https://github.com/MetaCubeX/mihomo">Mihomo</a>.
  <br/>
  Menu-bar first. SwiftUI. Zero compromise on smoothness.
</p>
...
<!-- TODO: drop a hero screenshot/GIF here once UI stabilizes -->
...
- **Concurrent latency tests** with animated colored badges. Batch and single-node testing
...
> No official release yet. Download pre-built versions from GitHub Releases, or build from source — see [Development](#development).
...
**mihomo binary lookup order** (priority high to low):
...
```
┌──────────────────────────────────────────┐
│  SwiftUI Views (MenuBar + MainWindow)    │
├──────────────────────────────────────────┤
│  AppModel (Global state, Observation)     │
├──────────────────────────────────────────┤
│  Service Layer                            │
│   ├─ CoreManager (mihomo process mgmt)    │
│   ├─ MihomoAPI (RESTful HTTP client)      │
│   ├─ TrafficMonitor (WebSocket traffic)   │
│   ├─ LogStream (WebSocket logs)           │
│   ├─ ConnectionMonitor (WebSocket conns)  │
│   ├─ SubscriptionService (fetch/parse)    │
...
> Status: M0–M2 complete. M4 polish mostly done. Awaiting M3 (TUN mode) and M5 (notarized release). See [落地计划.md](../落地计划.md) for the full milestone plan.

- [x] Menu-bar app (`MenuBarExtra`) with node switching, mode switching, and latency tests
...
- [x] Sparkle 2 auto-update (framework wired; needs `SUPublicEDKey` and an appcast feed)
...
[Sparkle 2](https://sparkle-project.org/) is integrated. Release flow:
